...
public class CinemaHallComfort extends CinemaHall{
    ...
    public CinemaHallComfort(Entertainment entertainment, int hall_area, int num_of_rows, int num_of_places, Cinema cinema, int money) {
        ...
        for (int i = 0; i < cinema.getNum_of_halls(); ++i){
            if (cinema.getOur_halls().get(i).getType().equals("Comfort")){
                ++num_of_hall;
            }
        }
    ...
    public void print_hall_info(){
        System.out.println("В кинозале установлены сверхудобные мягкие диванчики с подушками. В этом зале вы\n" +
                " почувствуете себя так же комфортно, как во время просмотра фильмов дома на любимом диване");
    }
